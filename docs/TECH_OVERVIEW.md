...
- `src/`
  ...
  - `core/`: **核心游戏逻辑**。
    ...
    - `systems/`: 核心子系统。
      ...
      - `generation/`: 场景生成子模块。Build/Game 场景现拆分为两条管线：旧 `ConstructionLayoutGenerator.js`（随机建筑外框 + BSP 切房，保留兼容）和新 `TownLayoutGenerator.js`（中心广场型聚落骨架 + 建筑模板装配）。新增 `TownDistrictPlanner.js`（广场/主街/街区规划）、`TownBlockAllocator.js`（按 block/density/strategy 分配建筑簇）、`BuildingTemplateLibrary.js`（建筑级模板库）、`RoomTemplateLibrary.js`（房间模板语义库）、`BuildingTemplateAssembler.js`（模板旋转、门位/墙体/家具装配）。地牢场景由 `DungeonLayoutGenerator.js` 负责，连接算法 **默认 V2（图优先拓扑 + 宏观网格嵌入，见 `generation/rooms/FloorTopology.js` 与 [room-v2:p3]）**：先规划有意图的图（主干 spine + 宝藏/商店/精英支线 + 至多 1 捷径环，三动词保底）→ 嵌入 24×20 cell 网格 → 房间物化/遭遇战定形 → 共享格边中点开门、3 宽短直走廊 → gate/掩体/装饰/火盆/火把/贴花/敌人配置。`?layout=v1` 回退旧 BSP 路径（中心工作区 BSP 切分→紧凑房间筛选→近邻约束 MST + 短环路→房间分类→内部模板→gate 放置），v1/v2 共用 `finalizeLayout` 收尾；`RoomInteriorTemplates.js` 提供 26 种房间内部布局模板（含特殊房专属与 Boss 三款）与按分类加权选择、`coverSpots` 建议掩体位。
    - `lighting/`: 像素光影子系统。
...
- **vsync 抖动吸附**：rAF 时间戳有 ±1~2ms 抖动，帧间隔与 k 个步长（k≤3）相差 <2ms 时按理想值累加，避免 60Hz 屏因"差一点攒不够一步"频繁跳帧卡顿（无吸附时实测显示 FPS 掉到 ~46）。
...
- 生成流水线（`DungeonLayoutGenerator.js`）：**连接默认走 V2（[room-v2:p3]）**——`FloorTopology.planFloorTopology` 图优先拓扑（主干 spine `起点→4~5 战斗/动词房→Boss 前厅→Boss` + 宝藏/商店/精英支线 + 至多 1 捷径环，房型规划期指定、三动词保底）→ 嵌入 24×20 cell 网格（4×5，自避行走 + 支线垂直外挂）→ 房间物化 + 遭遇战定形 → 门开共享格边重叠中点、3 宽短直走廊。`?layout=v1` 回退旧 BSP 路径（中心工作区 BSP 切分 → 紧凑房间筛选 → 近邻约束 MST + 1~2 短环路 → BFS 深度贴房型）。两路径**共用收尾** `finalizeLayout`：能量屏障 gate 放置在房间-走廊交界处 → 房间掩体（box/barrel/explosive_barrel，模板 `coverSpots` 建议位优先）与装饰（主题 `decorWeights` 加权 + 特殊房固定件：Boss 对称雕像/精英旗帜/宝箱房祭坛）+ 火盆布点（Boss 四角/精英对角/宝箱商店中轴侧）→ 壁挂火把布点（「南邻为地板」的墙面连续段等距）→ 地板贴花列表（房间散布 + 角落蛛网 + Boss 圆环 + 走廊稀疏）→ BFS 深度计算 → 按深度/分类/楼层分配敌人配置。产物含 `lightObjects`/`decals`，字段口径 v1/v2 完全一致。生成带质量门限与自动重试（V2 放不下自动降级 v1）。
...
- **房间系统 V2 · P1 分层中间表示（IR，[room-v2:p1]，详见 `docs/feature/ROOM_SYSTEM_V2.md`）**：引入 `generation/rooms/RoomPlan.js`——分层房间 IR（`mask` 形状层 / `floor` 逐格地板层 / `objects` 物件层[cover/decor/prop 三类] / `spawns` 出怪层[wave 1|2] / `pits` / `doorSlots` / `meta`），纯数据 + 纯函数（`createRoomPlan`/`validateRoomPlan`/`checkConnectivity`，无 Canvas）。字符模板经 `generation/rooms/EncounterAdapter.js` 的 `encounterToRoomPlan` 降级为「另一种授权前端」，一字不漏映射到 IR（`#`→mask 挖除 / `p`→pits / `c`→cover / `d`→decor / legend→prop / `floorType`→meta）。`selectEncounter` 返回 RoomPlan，`placeEncounter` **只吃 RoomPlan**（波次 1|2 在输出边界还原为下游 0|1；掩体/装饰保留 row-major 顺序，放置期 rng 序列不变），DungeonLayoutGenerator 消费链逐 tile 不变。`parseEncounter` 保留为旧口径兼容视图（底层同一适配器）。零回归由三重验证守护：黄金回归 `tests/room-plan-golden.test.js`（70/70 模板 → fixture `tests/fixtures/room-plan-golden.json` 逐坐标一致）、placeEncounter 新旧字节等价、全楼层布局 SHA-256 指纹（seed 7/42 × floor 1/2/3）改造前后完全一致。后续 P2 RoomBuilder（PixelDraw 式代码构建器）/P3 连接重做在此 IR 上开工。
- **房间系统 V2 · P2 代码化房间构建器（[room-v2:p2]，详见 `docs/feature/ROOM_SYSTEM_V2.md`）**：新增 `generation/rooms/RoomBuilder.js`——PixelDraw 式链式构建器，`defineRoom(id, opts, build(R))` 产「模板工厂」，`build(rng)` 每次用种子 rng 产一个变体（`count:[min,max]`/scatter 落点每局不同）。工具集：`R.shape`（rect/ellipse/carve/carveEllipse/union，任意形状）、`R.floor`（fill/rect/border/checker/scatter，逐格地板层 FLOOR_TYPES 键名→数值 id）、`R.objects`（place/cover/decor/row/ring/scatter，prop 确定件 + cover/decor 抽象件）、`R.spawns`（wave(1|2).at/ring/cluster，波次口径强制 1|2）、`R.pits`（rect/ring）、对称 `R.mirrorX/mirrorY/rot4`、`R.door`；坐标归一化（小数=相对尺寸）与绝对 tile 双写；产物过 `validateRoomPlan`，defineRoom 定义期 dry-run 即校验报错。选池接入：`EncounterTemplates.js` 中 `SELECTABLE_TEMPLATES = ENCOUNTER_TEMPLATES(纯字符池) + V2_SHOWCASE_ROOMS(5 代码房)`，`selectEncounter` 经统一接口（`templateGridSize`/`materializeTemplate`）同池加权选中，代码模板选中时用当局 rng 重新 build；新增 `selectEncounterById`。逐格地板落地：`placeEncounter` 产 `floorCells`（数值 id 反查 FLOOR_TYPES 键名；字符模板恒空=零回归），`DungeonLayoutGenerator` 转为 1×1 `floorOverrides` 置于整房 floorType 之后（逐格精确色胜出），实现「地板即像素画」。5 示范房 `generation/encounters/v2_showcase.js`（圆形大厅 F2/放射八向柱阵 F3/参数化柱廊 F1/同心环回廊 F2/渐变污渍大厅 F1，均字符画做不出的形态）。测试 `tests/room-builder.test.js`（40 例：工具单测/对称/归一化/wave 口径/校验报错/5 房 validate/双种子差异/入池选中/floorCells 落地）。
- **房间系统 V2 · P3 连接算法重做（[room-v2:p3]，详见 `docs/feature/ROOM_SYSTEM_V2.md` §6）**：连接从「BSP 撒房 + 随机 MST 环边 + 事后 BFS 贴房型」（病：拓扑无意图、门位无纪律、环边随机）重做为**图优先拓扑 + 宏观网格嵌入 + 开门纪律**。新增 `generation/rooms/FloorTopology.js`：`planTopologyGraph` 先规划有意图的抽象图——主干 spine（`起点→4~5 战斗/动词房→Boss 前厅→Boss`，房型规划期指定、三动词保底各 ≥1）+ 支线（宝藏~40%/商店~60%/精英随机深度，各垂直外挂）+ 至多 1 条捷径环（主干深处→前段，间隔 ≥3，不穿前厅/Boss/起点）；`embedTopology` 再把图嵌入 **24×20 cell 网格**（4 列×5 行，自避行走蛇形推进 + 支线垂直外挂 + 内部重试 40 次拉满可嵌入率）。`DungeonLayoutGenerator.js` 拆为 v1（旧 BSP，`?layout=v1` 回退）/v2（新，默认），**共用** `selectAndShrinkEncounters` + `finalizeLayout`（旧尾段逐字提取，rng 序列不变 → v1 等价）。开门纪律：门开在共享格边两房投影重叠段中点（`doorSlots` 优先），走廊恒为 **3 宽实心短直段**、各占相邻格间隙 → 永不斜穿/交叉。`generateDungeonLayout(...,{algorithm})` 默认 v2、放不下自动降级 v1（实测 600 布局 0 降级）。下游契约字段零变（rooms/corridors/gates/graph/floorOverrides…），`WorldSystem` 接 `?layout=v1`、`DungeonManager` 仅加兼容注释。测试 `tests/dungeon-topology.test.js`（10 例，24 seed×3 层不变量）；`dungeon-layout.test.js` 房数区间 8~10→9~12（旧区间是 BSP 叶子数副产物，V2 为有意图房数 10~11）。全量 518 + build 全绿。
