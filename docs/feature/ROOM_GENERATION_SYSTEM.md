...
按房间尺寸和入口关系分配语义：
1. 必分配：`living_room`、`bedroom`、`study`。
2. 剩余分配：`storage / corridor / foyer`。
3. 若缺少必需语义候选房间则本栋生成失败。

### 3.6 `FurniturePlacer.js`
按语义模板执行家具摆放：
...
### 3.7 `LayoutValidator.js`
生成结果统一校验：
1. 单栋内部房间图连通。
2. 入口门数量合法。
3. 家具无重叠且满足语义必选项。

### 3.8 `LayoutCompiler.js`
把 tile 计划转换为可实例化对象列表：
1. 墙、门、家具统一编译为 `{x, y, type}`。
2. 坐标转换为像素坐标（乘 `TILE_SIZE`）。

### 3.9 `ConstructionLayoutGenerator.js`
总编排器：
1. 组织各阶段执行。
2. 全局重试。
