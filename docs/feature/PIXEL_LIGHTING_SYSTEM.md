...
- `ShadowCasterBuilder.js`
  - 构建遮挡源缓存（墙体矩形 + 物体精灵 alpha 遮挡数据 + 动态实体遮挡数据）。
  - 物体遮挡默认使用**当前显示帧**的 alpha mask（非动画并集）。
  - 静态障碍（墙体/物体）仅在哈希变化时重建；动态实体（玩家/敌人/车辆）每帧刷新。
- `EntityLightOccluderResolver.js`
  - 将玩家/敌人/车辆的当前渲染帧解析为遮挡描述（支持翻转与旋转）。
...
3. 以离散角度发射射线，在遮挡场中逐像素步进：
   - 射线进入遮挡像素后继续步进，离开连续遮挡区时停在最后一个遮挡像素，确保遮挡像素本身受光且不向背后非像素区域传播。
...
- `floor_lamp`: 支持多色灯光预设（warm/cool/mint/rose），不同颜色台灯会发出对应光色并参与遮挡。
- `fish_tank`: 冷色弱光，支持阴影。
