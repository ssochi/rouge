...
*   **文件**: `src/assets/characters/enemies/soldier/SoldierGenerator.js`
*   **画布**: 32×32，默认朝左。
*   **定位**: 远程射击型敌人，使用 SMG 机枪进行 3 发点射，与 Hunter（手枪单发）形成武器差异。
*   **属性**: HP 60, Speed 0.9, Damage 5/发, Vision 350, ShootRange 200, MinRange 70。
*   **调色板**:
    *   **头盔**: 深橄榄绿 (#3d5c3a) / 暗色 (#2a3e28) / 碎片痕 (#4a6a47)。
    *   **面部**: 肤色 (#c4956a) / 迷彩涂装黑 (#2a3a1a) + 绿 (#4a5a3a)，严肃表情。
    *   **身体**: 迷彩底 (#5a6b4a) / 深绿补丁 (#3a4a2a) / 棕色补丁 (#6a5a3a)，黑色防弹背心 (#1a1a1a) + 肩带 (#2a2a2a)，弹药袋 (#4a3a2a)。
    *   **腿部**: 迷彩裤 (#5a6b4a) / 军靴 (#2a2a1a)。
*   **外观特征**:
    *   **头部**: 军用头盔 (深橄榄绿 + 弹片痕迹 + 护目镜凹槽 + 下巴扣带)，硬朗脸型，迷彩脸涂 (黑+绿条纹)，严肃紧闭嘴。
    *   **身体**: 迷彩上衣 (橄榄绿底 + 深绿/棕色迷彩补丁) + 黑色防弹背心 (前片 + 肩带) + 弹药袋。
    *   **无手臂绘制**: 武器与手由 `EnemyHandSystem` 运行时渲染 SMG。
    *   **腿部**: 标准 7 种姿势，迷彩裤 + 军靴。
*   **动画风格**:
    *   **Idle** (16帧): 微弱呼吸 (纪律感，幅度 0.5) + 头部缓慢左右扫描 (第 2-5 帧看左，第 10-13 帧看右，警戒感)。
    *   **Run** (12帧): 标准 Contact→Down→Pass→Up→Air→Reach 循环，头部略微前倾 (战术姿态)。
*   **AI 行为**:
    *   **点射机制**: 3 发连射 (burstSize=3)，每发间隔 8 帧 (burstDelayMax=8)，点射间冷却 160 帧 (burstCooldownMax=160)。
    *   **子弹散布**: ±0.08 rad 随机散布。
    *   **战斗横移**: combat 状态下沿垂直于玩家方向横移 (速度 0.5x)，每 ~90 帧切换方向。
    *   **后退**: 玩家过近 (< minRange) 时以 0.7x 速度后退。
